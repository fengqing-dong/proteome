...
#    title = ['GO-ID', 'Term', 'Category', 'FDR', 'P-Value', '#DIFF', '#REF', '%Diff', '%Ref', 'Over/Under', 'TestSeqs']
    ...
    def get_term_and_percent(bp_mf_cc,enrich_go):
        ...
        percent= list()
        try:
            filtered = filter(lambda line: line.split("\t")[-2] == "Over" and line.split("\t")[2] == bp_mf_cc and float(
                line.split("\t")[4]) < 0.05, open("./GO/{0}/enrichment.txt".format(group), "r"))
            sorted_temp = sorted(filtered, key=lambda s: float(s.split("\t")[4]))
            # for line in list(sorted_temp):
            for line in list(sorted_temp):
                if len(term) >= 10:
                    break
#                if line.split("\t")[1] in term:   # 去除重复
#                    continue
...
#    ax.set_xticklabels(range(0, int(p_max+1)+1), fontProperties=customer_font)
    if not int(p_max)>9:
        ax.set_xticks(range(0, int(p_max+1)+1))
    lengend_offset  = (3-(int(p_max+1) - (max(sorted(percent,reverse=True)[-3:])+1)*1.2)/int(p_max+1)*8.5)/8.4 +1
...
def txt2excel(group):
    ...
    if os.path.exists("./GO/{0}/topblast_youwu.txt".format(group)):
        ...
        command = "perl /home/fdong/auto_protein_analysis/TXToXLSX.pl {0}GO.xlsx {0}topblast.txt {0}protein2go.txt {0}BP.txt {0}MF.txt {0}CC.txt {0}enrichment.txt".format(path)
    os.system(command)
...
#       图片压缩
        ...
        png2.save("./GO/{0}/cluster_heatmap.tiff".format(group),compression="tiff_lzw",dpi=(600,600))
        png1.close()

        # 未经压缩
        # fig.savefig("./GO/{0}/cluster_heatmap.tiff".format(group),dpi=100)
...
#        txt2excel(group)
        if os.path.exists("./GO/{0}/cluster_diff_{0}.txt".format(group)) or os.path.exists("./GO/{0}/cluster_diff.txt".format(group)):
            ...
            path = "./GO/{0}".format(group)
            os.system("Rscript /home/fdong/auto_protein_analysis/heatmap.R -i {} -o {} ".format(file,path))
            # hcluster2(group)
        go_enrich(diff_multi,all_total,group)
        enrich_go_plot(group)
        txt2excel(group)
