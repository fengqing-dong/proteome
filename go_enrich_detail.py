...
def go_enrich(diff, all_total, group):
    ...
    enrich_file = open("./GO/{0}/enrichment_total.txt".format(group), "w")
...
def enrich_go_plot(group):
    ...
    def get_term_and_percent(bp_mf_cc, enrich_go):
        ...
        percent = list()
        try:
            filtered = filter(lambda line: line.split("\t")[-3] == "Over" and line.split("\t")[2] == bp_mf_cc and float(line.split("\t")[4]) < 0.05, open("./GO/{0}/enrichment_total.txt".format(group), "r"))
...
def txt2excel(group):
    ...
    if os.path.exists("./GO/{0}/topblast_youwu.txt".format(group)):
        command = "perl /home/fdong/auto_protein_analysis/TXToXLSX.pl {0}GO.xlsx {0}topblast.txt {0}protein2go.txt {0}BP.txt {0}MF.txt {0}CC.txt {0}enrichment_significant.txt {0}entichment_total.txt {0}enrichment_total_detail.txt {0}topblast_youwu.txt".format(path)
    else:
        command = "perl /home/fdong/auto_protein_analysis/TXToXLSX.pl {0}GO.xlsx {0}topblast.txt {0}protein2go.txt {0}BP.txt {0}MF.txt {0}CC.txt {0}enrichment_significant.txt {0}enrichment_total.txt {0}enrichment_total_detail.txt".format(path)
    os.system(command)
