...
def _enrich_go_plot(lengend_offset=None,axes_start=None):
...
#                    continue
    ...
    p_max = max(percent)
    width_user = round(len_max/88*11+12)
    ...
    fig = plt.figure()
    if not axes_start:
        axes_start = 0.88-8.5/width_user
    print("axes_start:\t",axes_start)
    ax = fig.add_axes([axes_start,0, 8.5/width_user, 0.98-1/height_user])
    #
    # ax = plt.axes()
    ...
    print("lengend_offset:\t",lengend_offset)
...
#    plt.legend([p3, p2, p1], ['Cellular component','Molecular function', "Biological process"], loc='lower right',frameon=True, fontsize=18, bbox_to_anchor=(lengend_offset, 0.035), borderaxespad=0.5)
    ...
    plt.savefig("enrich_go.tiff",dpi=150)
    plt.close()
