...
def enrich_go_plot(group):
    ...
    ax.set_yticklabels(Term, fontdict={'fontsize': 18}, fontProperties=customer_font)

    ax.set_xticks(range(0, int(p_max+1)+1))
    ax.set_xticklabels(range(0, int(p_max+1)+1), fontdict={'fontsize': 24}, fontProperties=customer_font)
    ...
    plt.title("-Log10(P_value)\n", fontProperties=customer_font, fontsize=24,fontweight = "bold")
...
#    p1 = ax.bar(np.arange(0, mf_num), nums[0:mf_num], color="#A52A2A", linestyle="-", edgecolor="black", linewidth=2)
#    p2 = ax.bar(np.arange(mf_num, bp_num), nums[mf_num:bp_num], color="#7570B3", linestyle="-", edgecolor="black", linewidth=2)
#    p3 = ax.bar(np.arange(bp_num, len(bp_mf_cc)), nums[bp_num:], color="#D95F02", linestyle="-", edgecolor="black", linewidth=2)
    ...
    ax.set_xticklabels(go_name, fontProperties=cutom_font, fontsize=14)
...
#    plt.legend([p1, p2, p3], ['Cellular component','Molecular function', "Biological process"], fontsize=16,loc= 9,bbox_to_anchor=(0.5, 1.2), ncol=3)
...
def hcluster2(group):
    ...
    def plot_cluster_heatmap_small(file):
        ...
        ax1 = fig.add_axes([0.1, 0.9, 0.8, 0.1])
        col_link = sch.linkage(df.T, method='average')
        z1 = sch.dendrogram(col_link, orientation='top', color_threshold=0, above_threshold_color="black", ax=ax1)
        col_name = df.columns[z1["leaves"]]
        ax1.set_axis_off()
        ...
        heatmap_data = df.values[z2["leaves"], :][:, z1["leaves"]]
        ax3 = fig.add_axes([0.1,1/pic_height, 0.8, 0.9-1/pic_height])
        mesh = ax3.pcolormesh(heatmap_data, cmap=newcmp,vmin=-2, vmax=2)
...
def main_process(group_names,all_annot_file,blast_top_hit,go_level_file):
...
if __name__ == "__main__":
    ...
    for i in range(2):
        if i==0:
            p = multiprocessing.Process(target=run_blast2go_cli,args=(taxid,))
            p.start()
        else:
            p2 = multiprocessing.Process(target=main_func,args=(taxid,email_info,))
            p2.start()
    # pool = multiprocessing.Pool(processes=2)
    # p1 = pool.apply_async(run_blast2go_cli, (taxid, )) # blast2go
    # p1.start()
    # p = pool.apply_async(main_func, (taxid,email_info ))  #kegg
    # p2.start()
    #  for  i in range(2):
     #   if i ==0:
      #      print("blast2go is running...") 
       #     pool.apply_async(run_blast2go_cli, (taxid, )) # blast2go
            # todo 衔接kegg_mapper_parse.py   #移动文件至KEGG文件夹
    # todo 衔接blast2go命令
    # all_annot_file = "来自blast2go命令结果"

        #else:
         #   print("kegg is running")
          #  pool.apply_async(main_func, (taxid,email_info ))  #kegg
    #pool.close()
    #pool.join()
