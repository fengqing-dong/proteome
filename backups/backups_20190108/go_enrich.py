...
def enrich_go_plot(group):
    ...
    plt.savefig("./GO/{0}/enrich_go.tif".format(group),dpi=150)
    plt.close()
...
def go_level2_plot(group,diff_multi):
    ...
    plt.savefig("./GO/{0}/GO_Level2.tif".format(group),dpi=150)
    plt.close()
...
def txt2excel(group):
    ...
    if os.path.exists("./GO/{0}/topblast_youwu.txt".format(group)):
        command = "perl /home/fdong/auto_protein_analysis/TXToXLSX.pl {0}GO.xlsx {0}topblast.txt {0}protein2go.txt {0}BP.txt {0}MF.txt {0}CC.txt {0}enrich.txt {0}topblast_youwu.txt".format(path)
    else:
        command = "perl /home/fdong/auto_protein_analysis/TXToXLSX.pl {0}GO.xlsx {0}topblast.txt {0}protein2go.txt {0}BP.txt {0}MF.txt {0}CC.txt {0}enrich.txt".format(path)
    os.system(command)
...
def main_process(group_names=None, all_annot_file=None,blast_top_hit=None, go_level_file=None):
    ...
    for group in group_names:
        ...
        txt2excel(group)
        if os.path.exists("./GO/{0}/cluster_diff_{0}.txt".format(group)) or os.path.exists("./GO/{0}/cluster_diff.txt"):
            hcluster2(group)
