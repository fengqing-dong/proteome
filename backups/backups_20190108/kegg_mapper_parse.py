...
#todo  txt文件直接放置到对应文件夹下 ---done
    if diff_or_all == "all":
        ...
        diff_group = diff_or_all.strip().split("_")[1]
        query2map = open("./KEGG_diff/{1}/query2map_{0}.txt".format(diff_or_all,diff_group), "w")
        mappids = open("./KEGG_diff/{1}/mappids_{0}.txt".format(diff_or_all,diff_group), "w")
        map2query = open("./KEGG_diff/{1}/map2query_{0}.txt".format(diff_or_all,diff_group), "w")

    query2map.write("ProteinID\tKO\tName\tDefinition\tMapID\tMapName\tURL\n")
    map2query.write("MapID\tMapName\tSeqs\t#Seqs\tURL\n")
...
# todo 多分组时文件夹生成和文件移动 ---done
    ...
    for file in os.listdir():
        if "all" in file and "fasta" not in file:
            try:
...
def enrich_kegg_plot(diff_multi):
    for diff_or_all in diff_multi.keys():
        ...
        plt.savefig("./KEGG_diff/{0}/Enrich_kegg.tif".format(diff_or_all),dpi=150)
        plt.close()
...
def main_func(taxid=None,email_info=None):
    # 任务开始
    # print(os.listdir("GO"))
    #print(os.getcwd())
    if os.path.exists("GO"):
